<template>
  <div>
    <!-- 多图片上传 -->
    <el-upload v-if="multiple" action='string' list-type="picture-card" :on-preview="handlePreview" :auto-upload="false"
               :on-remove="handleRemove" :http-request="upload"  :on-exceed="exceedImg" :on-change="consoleFL" :limit="maxLength" :file-list="uploadListNew">
      <i class="el-icon-plus"></i>
    </el-upload>
    <!-- 单图片上传 -->
    <el-upload v-else class="avatar-uploader" action="'string'" :auto-upload="false" :show-file-list="false"
    ...
    <!-- 剪裁组件弹窗 -->
    <el-dialog :visible.sync="cropperModel" width="800px" :before-close="beforeClose" append-to-body>
...
export default {
  ...
  methods: {
    ...
    exceedImg (files, fileList) {
      this.$message.error(`超出了上传文件的最大限度${this.maxLength}`)
    },
    handlePreview (file) {
      // 点击进行图片展示
    ...
    handleRemove (file, fileList) {
      ...
      console.log(this.uploadListNew);
